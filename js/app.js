...
async function loadDataFromJSON() {
  try {
    ...
    if (!productsRes.ok || !mitrasRes.ok) {
      throw new Error(`Failed to load data: products(${productsRes.status}), mitras(${mitrasRes.status})`);
    }

    const productsText = await productsRes.text();
    const mitrasText = await mitrasRes.text();
    
    // Validate and parse JSON
    products = JSON.parse(productsText);
    mitras = JSON.parse(mitrasText);
    ...
    localStorage.removeItem('warungProducts');
    localStorage.removeItem('warungMitras');
    products = [];
    mitras = [defaultMitra];
  }
}
